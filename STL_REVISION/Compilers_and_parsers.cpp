...
//     }
// };
// valid paranthesis modification problem, cool one
// bool check(string s,int i,int j){
//     stack<char>st;
//     for(int l=i;l<=j;l++){
//         if(s[l]=='<')st.push(s[l]);
//         else if(st.empty()||st.top()==s[l])return false;
//         else st.pop();
//     }
//     return st.empty();
// }
...
//             // res=max(res,j-i+1);
//             for(int j=i+1;j<n;j++){
//                 if(check(s,i,j))res=max(res,j-i+1);
//             }
//         }
//     }
//     cout<<res<<"\n";
// }     //HOLY COW!! this one's giving a TLE, gotta try something different,shid only passed half the cases.
void solve(){ // This is it!
    string s;
    cin>>s;
    int res=0,temp=0;
    stack<char>st;
